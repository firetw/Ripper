...
namespace WeiBoGrab
{
    ...
    public class FLoginSubmit
    {
        ...
        private bool Parser(WebBrowser browser)
        {
            ...
            if (browser.DocumentText.Contains("换一换"))
            {
                browser.Navigate(Core.FetionUrl);
                return result;
            }
            ...
            if (browser.DocumentText.Contains("<div class=\"type1\" style=\"\" action-type=\"patHorse1\"><a class=\"type1_click\" id=\"patHorse1\" style=\"cursor: url(/Public/images/activity/pat2.cur),auto;\"></a></div>"))
            {
                trigger.InvokeMember("click");
                Thread.Sleep(2000);
                Application.DoEvents();
            ...
            //if (browser.DocumentText.Contains("点击领取"))
            //{
            //    awardFlag = true;
            //}
            while (browser.ReadyState != WebBrowserReadyState.Complete)
            ...
            else
            {
                ...
                HtmlElementCollection tableCollection = browser.Document.GetElementsByTagName("TABLE");
                for (int i = 0; i < tableCollection.Count; i++)
                {
                    HtmlElement item = tableCollection[i];
                    if (item.GetAttribute("class") == "d-border")
                    {
                        awardRecoder.Log("WeiBoGrab.FLoginSubmit", "Parase", GetMsg(item.InnerText), Logger.LogLevel.INFO);
                        break;
                    }
                }
        ...
        private void LoginOut(WebBrowser browser)
        {
            ...
            browser.Navigate("http://gz.feixin.10086.cn/Bootlick/loginout");
